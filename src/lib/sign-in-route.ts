...
export function buildSignInSection({
  ...
  if (!environment.requiresApiKey) {
    lines.push(
      '',
      `This is a client-only app, so add a ${signInPath} client route that calls the SDK's signIn() as soon as the AuthKit client is ready, without a click (for example, in a useEffect). AuthKit keeps password-reset and invitation details through this redirect, so signIn() needs no extra arguments. If the app has a client router, register it there (for example { path: '${signInPath}', component: Login } or <Route path="${signInPath}" element={<Login />}>; Login must start sign-in on load). If it has none, compare window.location.pathname === '${signInPath}' at startup. The installer checks for one of these forms before it saves the Initiate login URI. Keep the existing sign-in button. Do not add a server route.`,
      '',
